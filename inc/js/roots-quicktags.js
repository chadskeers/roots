...
function edShowLinks() {
	var tempStr = '<select onchange="edQuickLink(this.options[this.selectedIndex].value, this);"><option value="-1" selected>' + '(Quick Links)' + '</option>', i;
...
function edInsertTag(myField, i) {
	...
	if (editor.getSelection() != "") { // Surround selected text with tags
		editor.replaceSelection(
			edButtons[i].tagStart +
			editor.getSelection() +
			edButtons[i].tagEnd
		);
	} else { // add the tag at the cursor
		if (!edCheckOpenTags(i) || edButtons[i].tagEnd == '') {
			editor.replaceSelection( edButtons[i].tagStart );
			edAddTag(i);
		}
		else {
			editor.replaceSelection( edButtons[i].tagEnd );
			edRemoveTag(i);
		}
		// Adding text this way leaves the text selected, so we have to move the cursor to the end
		// of the text manually
		var endposition = editor.getCursor(false);
		editor.setCursor(endposition);
	}
}

// Used for inserting arbitrary content, like the <img> tags which will always be closed
// upon insertion, so don't need to be handled by the tag stack.
// Reimplemented for roots theme
function edInsertContent(myField, myValue) {
	var editor = jQuery.editor;
	editor.focus();
	
	editor.replaceSelection( myValue );
	...
	if (!edCheckOpenTags(i)) {
		...
		if (URL) {
			edButtons[i].tagStart = '<a href="' 
				+ URL 
				+ '" title="' + prompt('Enter the link title', '')
				+ '">';
...
function edInsertImage(myField) {
	...
	if (myValue) {
		myValue = '<img src="'
				+ myValue
				+ '" alt="' + prompt('Enter the description of the image', '')
				+ '" />';
...
//	t.Buttons[t.Buttons.length] = new edButton(name+'_next','page','<!--nextpage-->','','p',-1);
	...
	html += '<input type="button" id="'+name+'_ed_spell" class="ed_button" onclick="edSpell('+name+'.Canvas);" title="' + 'Dictionary lookup' + '" value="' + 'close tags' + '" />';
	html += '<input type="button" id="'+name+'_ed_close" class="ed_button" onclick="'+name+'.edCloseAllTags();" title="' + 'Close all open tags' + '" value="' + 'close tags' + '" /></div>';
